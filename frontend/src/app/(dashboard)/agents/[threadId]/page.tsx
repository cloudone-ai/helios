...
export default function ThreadPage({ params }: { params: Promise<ThreadParams> }) {
  ...
  useEffect(() => {
    if (projectName) {
      ...
      document.title = `${projectName} | CloudOne Helios`;
      
      // Update meta tags for SEO
      const metaDescription = document.querySelector('meta[name="description"]');
      if (metaDescription) {
        metaDescription.setAttribute('content', `${projectName} - Interactive agent conversation powered by CloudOne Helios`);
      }
      
      // Update OpenGraph tags if they exist
      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | CloudOne Helios`);
      }
  ...
  const checkBillingStatus = useCallback(async () => {
    ...
    try {
      ...
      if (currentPlanId === SUBSCRIPTION_PLANS.FREE) {
        ...
        // The free plan has a 60 minute limit as defined in backend/utils/billing.py
        const FREE_PLAN_LIMIT_MINUTES = 60;
                        ...
                        <div key={group.key} ref={groupIndex === groupedMessages.length - 1 ? latestMessageRef : null}>
                          <div className="flex items-start gap-3">
                            <div className="flex-shrink-0 w-5 h-5 mt-2 rounded-md flex items-center justify-center overflow-hidden ml-auto mr-2">
                              <Image src="/CloudOne-symbol.svg" alt="CloudOne" width={14} height={14} className="object-contain invert dark:invert-0 opacity-70" />
                            </div>
                            <div className="flex-1">
