...
export default async function AccountBillingStatus({ accountId, returnUrl }: Props) {
    ...
    if (isLocalMode()) {
        return (
            <div className="rounded-xl border shadow-sm bg-card p-6">
                <h2 className="text-xl font-semibold mb-4">{t('billing.billingStatus')}</h2>
                <div className="p-4 mb-4 bg-muted/30 border border-border rounded-lg text-center">
                    <p className="text-sm text-muted-foreground">
                        {t('billing.localDevelopmentMode')}
                    </p>
                    <p className="text-xs text-muted-foreground mt-2">
                        {t('billing.agentUsageLimitsNotEnforced')}
                    </p>
                </div>
            </div>
        );
    ...
    let usageDisplay = t('billing.noUsageThisMonth');
    ...
    if (threadIds.length > 0) {
        ...
        if (agentRuns && agentRuns.length > 0) {
            ...
            usageDisplay = `${totalMinutes} ${t('billing.minutes')}`;
        }
    }
                        ...
                        <div className="rounded-lg border bg-background p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium text-foreground/90">{t('billing.currentPlan')}</span>
                                    <span className="text-sm font-medium text-card-title">{planName}</span>
                                </div>
                            </div>
                            
                            <div className="flex justify-between items-center">
                                <span className="text-sm font-medium text-foreground/90">{t('billing.agentUsageThisMonth')}</span>
                ...
                <>
                    <div className="mb-6">
                        <div className="rounded-lg border bg-background p-4 gap-4">
                            <div className="flex justify-between items-center">
                                <span className="text-sm font-medium text-foreground/90">{t('billing.currentPlan')}</span>
                                <span className="text-sm font-medium text-card-title">{t('billing.free')}</span>
                            </div>
