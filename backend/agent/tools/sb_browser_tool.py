...
class SandboxBrowserTool(SandboxToolsBase):
    ...
    async def _execute_browser_action(self, endpoint: str, params: dict = None, method: str = "POST") -> ToolResult:
        ...
        try:
            ...
            exit_code, response = self.sandbox.exec_cmd(curl_cmd)

            # response 可能为 bytes 或 str，需先 decode
            ...
            if exit_code == 0:
                try:
                    ...
                    logger.error(f"Failed to parse response JSON: {response_str} {e}")
                    return self.fail_response(f"Failed to parse response JSON: {response_str} {e}")
            else:
                logger.error(f"Browser automation request failed: exit_code={exit_code}, response={response_str}")
                return self.fail_response(f"Browser automation request failed: exit_code={exit_code}, response={response_str}")
