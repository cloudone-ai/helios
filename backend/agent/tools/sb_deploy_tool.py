    ...
    async def deploy(self, name: str, directory_path: str) -> ToolResult:
        ...
        try:
            ...
            try:
                ...
                exit_code, response = self.sandbox.exec_cmd(deploy_cmd)
                
                # 兼容 bytes 类型输出
                result_val = response.result
                if isinstance(result_val, bytes):
                    result_val = result_val.decode(errors="replace")
                print(f"Deployment command output: {result_val}")
                ...
                if response.exit_code == 0:
                    ...
                    return self.fail_response(f"Deployment failed with exit code {response.exit_code}: {result_val}")
            except Exception as e:
