...
async def get_or_create_project_sandbox(client, project_id: str):
    ...
    if not project.data or len(project.data) == 0:
        logger.error(f"Project {project_id} not found or empty response from DB: {project.data}")
        raise ValueError(f"Project {project_id} not found")
    project_data = project.data[0]

    # 检查 sandbox 字段的存在性和结构
    sandbox_info = project_data.get('sandbox')
    if isinstance(sandbox_info, dict) and sandbox_info.get('id') and sandbox_info.get('pass'):
        sandbox_id = sandbox_info['id']
        sandbox_pass = sandbox_info['pass']
        logger.info(f"Project {project_id} already has sandbox {sandbox_id}, retrieving it")
        try:
            sandbox = get_or_start_sandbox(sandbox_id)
            if not sandbox:
                logger.error(f"get_or_start_sandbox 返回 None, sandbox_id={sandbox_id}")
                raise Exception(f"get_or_start_sandbox 返回 None, sandbox_id={sandbox_id}")
        ...
        logger.warning(f"Project {project_id} 没有有效的 sandbox 字段，准备新建。sandbox_info={sandbox_info}")
    ...
    try:
        sandbox = create_sandbox(command=command, ports=ports, env=env)
    except Exception as e:
        logger.error(f"创建新sandbox失败: {e}")
        raise Exception(f"创建新sandbox失败: {e}")
    sandbox_id = sandbox.sandbox_id
    logger.info(f"Created new sandbox {sandbox_id}")

    # get_preview_link 也要加异常保护
    try:
        ...
        logger.error(f"get_preview_link 失败: {e}")
        vnc_url = None
        website_url = None
        token = None

    # 强制保证sandbox字段一定有id，日志增强
    sandbox_dict = {'id': sandbox_id}
    if sandbox_pass: sandbox_dict['pass'] = sandbox_pass
    if vnc_url: sandbox_dict['vnc_preview'] = vnc_url
    if website_url: sandbox_dict['sandbox_url'] = website_url
    if token: sandbox_dict['token'] = token
    logger.info(f"Updating project {project_id} with sandbox: {sandbox_dict}")
    update_result = await client.table('projects').update({
        'sandbox': sandbox_dict
    }).eq('project_id', project_id).execute()
    # 写入后立即select校验
    verify = await client.table('projects').select('sandbox').eq('project_id', project_id).execute()
    logger.info(f"Post-update sandbox field: {verify.data}")
    if not update_result.data or len(update_result.data) == 0:
        logger.error(f"Failed to update project {project_id} with new sandbox {sandbox_id}, update_result: {update_result}")
...
async def start_agent(
    ...
    if not thread_result.data or len(thread_result.data) == 0:
        logger.error(f"Thread {thread_id} not found or empty response from DB: {thread_result.data}")
    ...
    if not agent_run.data or len(agent_run.data) == 0:
        logger.error(f"Failed to create agent run for thread {thread_id}, response: {agent_run}")
